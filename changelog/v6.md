...
- ECONREFUSED error for noode is fixed with this version.

## 6.3.0 (Feb 6th, 2024)

### Fixed

- terser-webpack-plugin error during plugin build is fixed with upgraded package.
- source-map file generation issue is addressed.
- 1 critical and 4 high vulnerabilities in packages have been addressed.
- Twilio Serverless build API now uses 'latest' node runtime
- webpack is reverted to latest stable v4 (4.47.0) as many of the dependencies that plugins use are not webpack 5 ready yet.
