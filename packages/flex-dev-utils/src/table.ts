...
export const printArray = (header: string[], data: string[][]) => {
  if (!isRegularMatrix(data)) {
    logger.error('Table rows are not all the same length; this may produce irregular tabular view.');
  }

  if (header.length !== Object.keys(data[0]).length) {
    logger.error('Header length does not match data row length; printing table without header');
  } else {
