...
describe('table', () => {
  ...
  describe('isRegularMatrix', () => {
    ...
    it('should return true for valid matrix', () => {
      ...
      expect(table.isRegularMatrix([['a', 'b', 'c'], ['e', 'f', 'g']])).toBeTruthy();
    });

    it('should return true for invalid matrix', () => {
      expect(table.isRegularMatrix([['a', 'b', 'c'], ['e', 'f']])).toBeFalsy();
    });
  });
  ...
  describe('printObjectArray', () => {
    ...
    it('should call printArray', () => {
      const printArray = jest.spyOn(table, 'printArray').mockImplementation(() => { /* no-op */ });
